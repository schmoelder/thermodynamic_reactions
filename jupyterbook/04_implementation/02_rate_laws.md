...
## Temperature-independent rate constants break equilibrium consistency

The ratio $k_f/k_r$ determines the equilibrium composition only at the temperature where the parameters were fitted.
Treating $k_f$ and $k_r$ as constants therefore fixes their ratio and implicitly assumes a temperature-independent equilibrium constant, which is inconsistent with thermodynamics.

To introduce temperature dependence, $k_f(T)$ and $k_r(T)$ must satisfy $k_f(T)/k_r(T) = K(T)$ at all temperatures; otherwise, the model converges to incorrect equilibria when $T$ changes (@fig-rate-drift).
This is the kinetic analogue of using a constant $K$ in equilibrium calculations.
`ThermodynamicReaction` resolves this by enforcing $k_r(T) = k_f(T)/K(T)$.

```{code-cell} ipython3
:tags: [remove-cell]
:label: cell-rate-drift

from reactions.api import EquilibriumConstantVantHoff

K_thermo = EquilibriumConstantVantHoff(dH=-20e3, dS=-50.0)
...
kf_ref = 2.0
kr_ref = kf_ref / K_T1   # calibrated at T1
c_ref  = 1000.0

# MassActionReaction is temperature-independent; same trajectory at both T
A_eq_fixed = c_ref / (1 + K_T1)   # where kf/kr actually sends the system
B_eq_fixed = c_ref - A_eq_fixed

A_eq_T2 = c_ref / (1 + K_T2)      # where the system should go at T2
B_eq_T2 = c_ref - A_eq_T2

t_sim = np.linspace(0, 10.0, 300)
tau   = 1.0 / (kf_ref + kr_ref)
B_kin = B_eq_fixed * (1 - np.exp(-t_sim / tau))

fig, axes = plt.subplots(1, 2, figsize=(9, 3.8), sharey=True)

for ax, T, B_true, title in [
    (axes[0], T1, B_eq_fixed, f"$T = {T1 - 273.15:.0f}\ °\\mathrm{{C}}$ (calibration)"),
    (axes[1], T2, B_eq_T2,    f"$T = {T2 - 273.15:.0f}\ °\\mathrm{{C}}$"),
]:
    ax.plot(t_sim, B_kin, color="C1", label="B (simulated)")
    ax.axhline(B_eq_fixed, color="C1", lw=1.0, ls="--", label=f"$k_f/k_r$ equilibrium")
    ax.axhline(B_true,     color="C2", lw=1.0, ls=":",  label=f"$K(T)$ equilibrium")
    ax.set_xlabel("time [s]")
    ax.set_title(title)
    ax.legend(fontsize=8)

axes[0].set_ylabel("$c_B$ [mol/m³]")
fig.tight_layout()
```

```{figure} #cell-rate-drift
:name: fig-rate-drift

A⇌B with `MassActionReaction` ($k_f = 2\ \text{s}^{-1}$, $k_r = k_f/K(25\ °\text{C})$).
At the calibration temperature (left) the long-time limit matches $K(T)$ exactly.
At $T = 67\ °\text{C}$ (right) the system converges to the same $k_f/k_r$ (dashed) while the true $K(T)$ has shifted to a lower value (dotted); the simulation produces the wrong equilibrium.
`ThermodynamicReaction` with `EquilibriumConstantVantHoff` eliminates this error.
```

---

The next chapter adds that constraint: `ThermodynamicReaction` derives $k_r = k_f / K(T)$ at every temperature so the long-time limit tracks the true equilibrium exactly (@implementation-equilibrium).
